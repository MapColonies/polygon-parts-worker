...
import { TaskHandler as QueueClient, IUpdateJobBody } from '@map-colonies/mc-priority-queue';
import { Tracer } from '@opentelemetry/api';
import { withSpanAsyncV4 } from '@map-colonies/telemetry';
import { PolygonPartsEntityName } from '@map-colonies/mc-model-types';
import { SERVICES } from '../common/constants';
import { IConfig, IJobAndTaskResponse, IPermittedJobTypes, JobProfile, PolygonPartsPayloadAndAdditionalParams } from '../common/interfaces';
...
export class JobProcessor {
    ...
    while (this.isRunning) {
      ...
      try {
        ...
        if (jobAndTask) {
          ...
          const updatedJob: IUpdateJobBody<PolygonPartsPayloadAndAdditionalParams> = this.adjustJobToPolygonPartsManager(job, polygonPartsEntity);
          this.logger.info({ msg: 'updating additionalParams for job', jobId: job.id });
          await this.queueClient.jobManagerClient.updateJob(job.id, updatedJob);
  ...
  private async getJobAndTask(): Promise<IJobAndTaskResponse | undefined> {
    ...
    for (const jobType of jobTypesToProcessArray) {
      ...
      if (task) {
        ...
        const job = await this.queueClient.jobManagerClient.getJob<PolygonPartsPayloadAndAdditionalParams, unknown>(task.jobId);
  ...
  private adjustJobToPolygonPartsManager(
    job: JobProfile,
    polygonPartsEntity: PolygonPartsEntityName
  ): IUpdateJobBody<PolygonPartsPayloadAndAdditionalParams> {
    const newAdditionalParameters = { ...job.parameters.additionalParams, ...polygonPartsEntity };
    const newParameters = { ...job.parameters, ...newAdditionalParameters };
    return { ...job, parameters: newParameters };
  }
}
